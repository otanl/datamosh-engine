...
static void runCuda(int idx, const std::vector<uchar4>& in, std::vector<uchar4>& out)
{
    DatamoshDctCudaParams p = dctcuda::presetParams(idx);
    dctcuda::applyControls(p, 1, 1, 1, 1, 1);
...
int main()
{
    ...
    printf("CPU<->CUDA DCT0 parity (MAE per preset, threshold %.1f)\n", THRESHOLD);
