...
use datamosh::{DctCodec, DctCodecConfig, DctGlitchParams, load_dct_transform_preset};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let preset = args.get(3).map(String::as_str).unwrap_or("composite");
    ...
    load_dct_transform_preset(preset, &mut params)?;
    let mut codec = DctCodec::new(DctCodecConfig::new(width, height))?;
    ...
    println!(
        "{width}x{height} preset={preset} frames={frames} elapsed={elapsed:.3}s fps={:.2} ms/frame={:.2} checksum={checksum}",
