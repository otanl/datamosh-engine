...
// CLI subcommand (motion codec only), this reaches all three engine backends —
// handy for generating side-by-side demo clips without a plugin host:
//
//   ffmpeg ... -f rawvideo -pix_fmt rgb24 - \
//     | cargo run --release --example raw_engine -- <w> <h> <backend 1|2|3> <preset>
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let width: usize = args
        .first()
        .ok_or("usage: raw_engine <width> <height> <backend 1|2|3> <preset>")?
        .parse()?;
    ...
    let backend = MoshEngineBackend::parse_id(backend_id)
        .ok_or("backend must be 1 (motion), 2 (scanline), or 3 (dct)")?;
    let frame_len = width
        .checked_mul(height)
