...
$srcDir = Join-Path $root 'touchdesigner\DatamoshDctCudaTOP'
$harness = Join-Path $root 'tools\dct_parity_check.cu'
$kernels = Join-Path $srcDir 'DatamoshDctCudaKernels.cu'
$includeDir = Join-Path $root 'include'
$outputDir = Join-Path $root 'target\release'
$exe = Join-Path $outputDir 'dct_parity_check.exe'
$lib = Join-Path $outputDir 'datamosh.dll.lib' # cdylib import lib (matches dllimport in the header)
