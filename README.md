...
*The same Mandelbrot zoom pushed through the three original custom codec backends: **MSH0 motion** (`classic` — stale motion vectors with residuals dropped, so it melts), **SCN0 scanline/signal** (`predictor-ghost` — analog-style line tearing and ghosting), and **DCT0 transform** (`bleed` — JPEG-style block DC smear). The newer **WVT0 wavelet** backend adds multiscale subband and lifting-state failure. GPU-native CUDA TOPs are available for the motion, DCT, and WVT0 paths.*
...
Its core is one FFmpeg → engine → FFmpeg pipe per backend (`raw_engine` arg 3 selects the backend: 1 = motion, 2 = scanline, 3 = dct, 4 = wavelet):
...
- **Custom codecs** (the focus) — tiny video codecs you encode into and then *damage on decode*. Four backends, each a different glitch family, all driven through one `MoshEngine` object and exposed over a C ABI for plugin hosts:
  ...
  - **WVT0** (backend `4`) — a multilevel integer Haar lifting codec with YCoCg-R colour transform, quantized subband packets, packet routing, bitplane, hierarchy, and temporal subband-history corruption.
...
All four backends run through `MoshEngine` (one engine per host node) and take/return raw `rgb24` or `rgba8`. This is the path plugin hosts use; it is a low-latency, editable codec representation that can be damaged safely.
...
| **WVT0** wavelet (`wavelet_pyramid_v1`) | `4` | clean, subband-slip, orientation-cross, scale-fold, bitplane-rain, lowpass-ghost, temporal-weave, packet-loss, lifting-drift, chroma-pyramid, hierarchy-collapse |
...
- **WVT0** converts RGB with reversible YCoCg-R, applies a multilevel integer Haar lifting transform, and packetizes independently quantized approximation/horizontal/vertical/diagonal subbands. Glitches misroute scales and orientations, alter coefficient bitplanes, lose or conceal packets from history, retain stale lowpass state, and bias the inverse lifting stage. Quality `100` is exactly reversible; row/column lifting and subband reconstruction are Rayon-parallel. The RGB conversion is fused with the outermost lifting stage, packet storage is recycled, and optional rate estimation is kept off the realtime path.
...
cargo run --release --example wavelet_codec_bench  -- 640 360 120 hierarchy-collapse
```

Append `rate` to the wavelet benchmark to include the diagnostic entropy-size estimate; realtime processing leaves that extra scan disabled.
...
Backend constants: `1` = `RAW_MOSH_V1`, `2` = `SCANLINE_SIGNAL_V1`, `3` = `DCT_TRANSFORM_V1`, `4` = `WAVELET_PYRAMID_V1`. Add future codecs as new backend IDs, not new entry points. Full declarations are in [`include/datamosh_ffi.h`](include/datamosh_ffi.h); a dynamic-load smoke host builds with `.\scripts\build-cpp-smoke.cmd -Run`.
...
- CPU TOPs `DatamoshTOP` / `DatamoshScanlineTOP` / `DatamoshDctTOP` / `DatamoshWaveletTOP` (backends 1/2/3/4) load `datamosh.dll` beside them. They download input as `RGBA8Fixed`, process the previous frame (≈1-frame latency, avoids a same-frame GPU readback stall), and upload `RGBA8Fixed`.
- Three **CUDA TOPs** (`DatamoshCudaTOP`, `DatamoshDctCudaTOP`, `DatamoshWaveletCudaTOP`) are separate GPU-native re-implementations that keep all state on-GPU and do **not** link `datamosh.dll`. Parity with the CPU codecs is maintained by hand. DCT0 and WVT0 have automated guards (`.\scripts\build-dct-parity-check.cmd`, `.\scripts\build-wavelet-parity-check.cmd`).
- The CPU and CUDA WVT0 TOPs expose the same preset menu, codec settings, detailed overrides, Advanced parameter override, Audio CHOP mapping, and reset controls.
- The CPU and CUDA DCT0 TOPs share the coefficient-domain controls and host parameter surface. The serialized DTE0 entropy path remains CPU-only.
- The Motion CUDA TOP exposes the CPU Motion TOP's 18 visible detailed controls as GPU-native equivalents, plus the same Advanced and Audio host controls. Its MSH0-like byte, coefficient, and codebook operations remain a parallel CUDA implementation rather than a serialized CPU bitstream round trip.
...
Key files: `src/mosh_codec.rs` (MSH0), `src/scanline_codec.rs` (SCN0), `src/dct_codec.rs` + `src/dct_bitstream.rs` (DCT0 / DTE0), `src/wavelet_codec.rs` (WVT0), `include/datamosh_ffi.h` (C ABI), `touchdesigner/` (the TD bridges).
